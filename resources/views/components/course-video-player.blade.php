@props(['video', 'course', 'poster' => null, 'watermark' => null, 'retryable' => false, 'autoplay' => false])
...
@if ($video->status === \App\Enums\VideoStatus::Ready && $video->hls_path)
    <div wire:ignore class="course-video-player" data-course-video-player data-manifest="{{ route('course-videos.stream.master', [$course, $video]) }}" data-autoplay="{{ $autoplay ? 'true' : 'false' }}">
        <video playsinline preload="metadata" @if($poster) poster="{{ $poster }}" @endif></video>
        @if ($watermark)
...
@once
    <style>
        .course-video-player{position:relative;aspect-ratio:16/9;overflow:hidden;border-radius:1rem;background:#050505;--shaka-primary-color:#f59e0b;--shaka-secondary-color:#fff}.course-video-player video{width:100%;height:100%;object-fit:contain}.course-video-player .shaka-controls-container{font-family:inherit}.course-video-player .shaka-overflow-menu,.course-video-player .shaka-settings-menu{border-radius:.8rem;background:rgba(17,24,39,.97);color:#fff}.course-video-player .shaka-overflow-button-label,.course-video-player .shaka-current-selection-span{color:#fbbf24}.course-video-watermark{position:absolute;z-index:2;top:1rem;right:1rem;max-width:30%;pointer-events:none;color:#fff;font-weight:700;opacity:.35;text-shadow:0 1px 4px #000}.course-video-playback-error{position:absolute;z-index:4;right:1rem;bottom:4.5rem;left:1rem;padding:.7rem 1rem;border-radius:.7rem;background:rgba(127,29,29,.92);pointer-events:none;text-align:center;color:#fff;font-size:.82rem;font-weight:700}.course-video-state{display:flex;aspect-ratio:16/9;flex-direction:column;align-items:center;justify-content:center;gap:.75rem;border-radius:1rem;background:#111827;padding:2rem;text-align:center;color:#e5e7eb}.course-video-state span{color:#9ca3af}.course-video-state-error strong{color:#fca5a5}.course-video-state button{border-radius:.65rem;background:#f59e0b;padding:.65rem 1rem;font-weight:800;color:#111827}.course-video-spinner{width:2rem;height:2rem;border:3px solid #ffffff2e;border-top-color:#f59e0b;border-radius:50%;animation:course-video-spin .8s linear infinite}@keyframes course-video-spin{to{transform:rotate(360deg)}}
    </style>
@endonce
