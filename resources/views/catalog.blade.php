...
@endphp
<header class="header"><div class="container nav"><a class="brand" href="{{ route('home') }}"><img src="{{ asset('images/learning-platform-logo.png') }}" alt=""><b>{{ config('app.name', 'EduPath') }}</b></a><nav><a href="{{ route('home') }}">{{ $ar ? 'الرئيسية' : 'Home' }}</a><a href="{{ route('categories.index') }}">{{ $ar ? 'التصنيفات' : 'Categories' }}</a><a href="{{ route('courses.latest') }}">{{ $ar ? 'الكورسات' : 'Courses' }}</a></nav><div class="nav-actions"><a class="lang" href="{{ route('locale.switch', $ar ? 'en' : 'ar') }}">{{ $ar ? 'EN' : 'ع' }}</a><a class="login" href="{{ route('filament.students.auth.login') }}">{{ $ar ? 'دخول' : 'Sign in' }}</a><a class="btn small" href="{{ route('filament.students.auth.register') }}">{{ $ar ? 'ابدأ الآن' : 'Get started' }}</a></div></div></header>
<main>
