@php
    ...
    $inputId = 'r2-attachment-'.md5($getStatePath());
@endphp

<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    <div x-data="courseAttachmentUploader({ baseUrl: @js($uploadBaseUrl), statePath: @js($parentStatePath), csrfToken: @js(csrf_token()) })" class="space-y-3">
        @if ($uploadBaseUrl)
            <input id="{{ $inputId }}" type="file" accept=".pdf,.zip,.rar,.7z,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.txt,.csv" x-on:change="selectFile($event)" x-bind:disabled="uploading" class="sr-only" />
            <label for="{{ $inputId }}" class="inline-flex cursor-pointer rounded-lg bg-gray-600 px-3 py-2 text-sm font-semibold text-white hover:bg-gray-500">Choose attachment file</label>
            <p x-show="fileName" class="text-sm text-gray-600 dark:text-gray-300"><span x-text="fileName"></span> <span x-text="fileSize ? `(${fileSize})` : ''"></span></p>
            <button type="button" x-on:click="upload()" x-bind:disabled="! file || uploading" class="fi-btn rounded-lg bg-primary-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50"><span x-show="! uploading">Upload attachment to R2</span><span x-show="uploading" x-text="`Uploading ${progress}%`"></span></button>
            <div x-show="uploading || progress > 0" class="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900/50"><div class="mb-2 flex items-center justify-between gap-4"><span class="text-sm font-medium text-gray-700 dark:text-gray-200" x-text="uploading ? 'Uploading attachment...' : 'Upload complete'"></span><span class="text-lg font-bold text-primary-600" x-text="`${progress}%`"></span></div><div class="h-3 overflow-hidden rounded-full bg-gray-200 shadow-inner dark:bg-gray-700"><div class="h-full rounded-full bg-gradient-to-r from-primary-500 to-success-500 transition-all duration-300" x-bind:style="`width: ${progress}%`"></div></div></div>
        ...
        @else
            <p class="text-sm text-warning-600">Save the course first, then edit it to upload attachments directly to R2.</p>
        @endif
    </div>
</x-dynamic-component>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('courseAttachmentUploader', ({ baseUrl, statePath, csrfToken }) => ({
        ...
        async upload() { if (! this.file || ! this.baseUrl || this.uploading) return; this.uploading = true; this.progress = 0; this.status = ''; this.error = ''; this.partProgress = {}; let upload = null; try { upload = await this.request('/initiate-attachment', { file_name: this.file.name, file_size: this.file.size, content_type: this.file.type || 'application/octet-stream' }); const parts = await this.uploadParts(upload); const result = await this.request('/complete', { key: upload.key, upload_id: upload.upload_id, parts }); this.$wire.set(`${this.statePath}.file_name`, this.file.name, false); this.$wire.set(`${this.statePath}.file_path`, result.key, false); this.$wire.set(`${this.statePath}.file_size`, this.file.size, false); this.progress = 100; this.status = 'Attachment uploaded successfully.'; } catch (error) { if (upload?.upload_id) await this.abort(upload).catch(() => {}); this.error = error.message || 'The attachment upload failed.'; } finally { this.uploading = false; } },
        ...
        formatBytes(bytes) { if (! bytes) return '0 B'; const units = ['B', 'KB', 'MB', 'GB']; const unit = Math.min(Math.floor(Math.log(bytes) / Math.log(1024)), units.length - 1); return `${(bytes / (1024 ** unit)).toFixed(unit ? 2 : 0)} ${units[unit]}`; },
    }));
});
</script>
@endif
